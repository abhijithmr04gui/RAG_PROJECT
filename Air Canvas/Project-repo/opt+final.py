...
pygame.mixer.init()
ALERT_SOUND = "/Users/abhijithmr/Downloads/beep.mp3"

# Dlib face and landmark detector
detector = dlib.get_frontal_face_detector()
predictor = dlib.shape_predictor("/Users/abhijithmr/Downloads/shape_predictor_68_face_landmarks.dat")
...
for text, (x1, y1, x2, y2), color in buttons:
    ...
    cv2.putText(paintWindow, text, (x1 + 10, 33), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 2)
...
with mpHands.Hands(max_num_hands=1, min_detection_confidence=0.7) as hands:
    while True:
        ...
        # Process each face
        for rect in rects:
            rect = dlib.rectangle(left=int(rect.left() * 2), top=int(rect.top() * 2),
                                  right=int(rect.right() * 2), bottom=int(rect.bottom() * 2))
            ...
            if ear < EAR_THRESHOLD:
                ...
                if COUNTER >= CONSEC_FRAMES:
                    cv2.putText(frame, "DROWSINESS ALERT!", (150, 400), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
                    if not ALARM_ON:
        ...
        for text, (x1, y1, x2, y2), color in buttons:
            ...
            cv2.putText(frame, text, (x1 + 10, 33), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,0,0), 2)
        ...
        if result.multi_hand_landmarks:
            for handlms in result.multi_hand_landmarks:
                lmList = [(int(lm.x * 640), int(lm.y * 480)) for lm in handlms.landmark]
            ...
            if len(bpoints[blue_index]) != 0: bpoints.append(deque(maxlen=512)); blue_index += 1
            if len(gpoints[green_index]) != 0: gpoints.append(deque(maxlen=512)); green_index += 1
            if len(rpoints[red_index]) != 0: rpoints.append(deque(maxlen=512)); red_index += 1
            if len(ypoints[yellow_index]) != 0: ypoints.append(deque(maxlen=512)); yellow_index += 1
            ...
            for j in range(len(pointGroup)):
                for k in range(1, len(pointGroup[j])):
                    if pointGroup[j][k - 1] and pointGroup[j][k]:
                        cv2.line(frame, pointGroup[j][k - 1], pointGroup[j][k], colors[i], 2, cv2.LINE_AA)
                        cv2.line(paintWindow, pointGroup[j][k - 1], pointGroup[j][k], colors[i], 2, cv2.LINE_AA)

        # OCR every OCR_INTERVAL frames
        if ocr_counter % OCR_INTERVAL == 0:
            ...
            if extracted_text.strip():
                try:
                    ocr_result = eval(extracted_text.strip())
        ...
        if ocr_result is not None:
            cv2.putText(frame, f"Result: {ocr_result}", (150, 450), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 3)
